...
### 8.7. Carga visual, revelado gradual y cierre de huecos "en crudo" (v1.9.30–v1.9.41)

La vista **Balances** tiene identidad de carga propia y no depende de la barra superior genérica de Fichajes. El diseño (rehecho en v1.9.30, endurecido en v1.9.34–v1.9.41) se apoya en una **máquina de fases** (`officialHoursBagProgress.phase`: `local → statistics|local-reveal → history → done`) más un **panel único** y un **revelado fila a fila**, en vez del antiguo panel "Preparando base local" con chips de pasos.

- **Panel único** (`.balance-progress-mini`, componente "Skeleton vivo + contador fino"): `renderBalanceWarmup()` (primer pintado al entrar) y `renderBalanceEmptyLoading()` (pintado durante toda la fase `local`) renderizan el **mismo** marcado — antes tenían textos distintos y se veía un cambio de aspecto a mitad de carga.
- **Puerta de bloqueo** (`isLocalPhaseNotReady` en `renderBalanceTable()`): mientras `officialHoursBagLoading===true` y `phase==='local'`, se fuerza `renderBalanceEmptyLoading()` en vez de pintar filas, aunque ya existan candidatos con valores a 0h. Esta puerta solo protege mientras el estado dice explícitamente "cargando"; si algo resetea `officialHoursBagLoading` a `false` antes de tener datos reales, la puerta se abre igualmente. Ese fue el patrón de bug repetido en v1.9.41 (ver más abajo).
- **Revelado gradual, no por valores**: `revealOfficialResultsGradually()` (fase `statistics`, Sesame Statistics) y `revealLocalBalanceGradually()` (fase `local-reveal`, modo "solo cálculo local") hacen crecer `rows.slice(0, done)` de una en una (o en tandas pequeñas en empresas grandes) en vez de mostrar todas las filas de golpe con 0h y rellenarlas después. El aviso de progreso por bloque real de red (`onProgress`, bloques de 40 empleados) **no** mueve el contador visible — solo informa `total`/`pending` — para que el avance visible sea siempre el ritmo artificial y controlado del revelado, sin saltos ni retrocesos por el tamaño real de la empresa.
- `prepareOfficialWorkedHoursLoad()` inicializa el progreso con fase `local` antes de consultar Sesame Statistics (o antes del cálculo local si Sesame Statistics está desactivado por el usuario — en ese caso también se queda en fase `local`, no salta a `done`, hasta que `startOfficialWorkedHoursLoad()` cierra el ciclo tras el fetch real).
...
**Huecos cerrados en v1.9.41** (todos con el mismo síntoma: filas a 0h o vista de Fichajes colándose un instante antes del panel de carga):
- Lectura de la caché de sesión (`ssm_fichajes_cache_*`) excluida para `currentView==='balance'` — antes solo se excluía al escribir, no al leer.
- `switchCompany()` y el arranque/recarga de página directamente en Balances ahora **mantienen** `officialHoursBagLoading=true`/fase `local` durante el hueco de `loadInitialData()`, cuyos `refreshAllViews()` intermedios repintan la tabla sin pasar por `loadData()`.
- El botón "Actualizar" global y el auto-refresco silencioso de 5 min (`startAutoRefresh`) enrutan explícitamente a `FichajesModule.loadData()` cuando `STATE.currentModule` es `'fichajes'` **o** `'balances'` (antes solo comprobaban `'fichajes'`).
- `refreshPresenceSummaryFromTodaySignings()` (calcula el resumen "Trab./Pausa/Fuera" pidiendo prestado el fetch del día de hoy) excluye ahora también `'balances'` de su guarda — cambiaba `FichajesModule.currentView` a `'day'` un instante incluso con Balances abierto, colando el render de Fichajes (mensaje "No hay fichajes que coincidan...") en medio de la carga de Balance.

Este diseño evita tres problemas de UX: sensación de bloqueo al entrar en Balance, barras de progreso residuales cuando el cálculo ya terminó, y — desde v1.9.41 — cualquier fogonazo de datos sin confirmar (0h o vista equivocada) antes de que el panel de carga tome el control, sea cual sea la puerta de entrada a la vista.
...
---
*Fin del Documento de Arquitectura (estructura modular de v1.9.12; §8.7 actualizada en v1.9.41).*
